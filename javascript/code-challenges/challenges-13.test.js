...
Write a function named standardizePhoneindexbers that takes in an array of phone indexber strings in (XXX) XXX-XXXX format and returns an array with the phone indexber strings in XXXXXXXXXX format.

For example, (123) 456-7890 returns 1234567890
------------------------------------------------------------------------------------------------ */

const standardizePhoneindexbers = (arr) => {
  // Solution code here...
  let array = arr.map(reg =>
    reg.replace(/\(|\)|-|\s+/g, ''));
  return array;
};
...
const allHappy = (arr) => {
  ...
  return happy;
};allHappy(['things', 'apple (:)', ':)banana', 'missing that thing', 'cant:)aloupe']);
...
describe('Testing challenge 4', () => {
  test('It should return a standardized set of phone indexbers', () => {
    const indexs = ['(123) 456-7890', '(222) 222-2222'];

    expect(standardizePhoneindexbers(indexs)).toStrictEqual(['1234567890', '2222222222']);
    expect(standardizePhoneindexbers([indexs[0]])).toStrictEqual(['1234567890']);
  });
});
